...
def eval(state):
    ...
    return fin+manhattanDistance(state.getPacmanPosition(), state.getGhostPosition(1))-50*state.getFood().depth() - 4*md1 - 4*md
...
def mini(state, closed, depth):
    ...
    if depth == 8 or terminal_state(state):
        return eval(state)
...
class PacmanAgent(Agent):
    ...
    def get_action(self, state):
        ...
        """
        self.move = self.hminimax2(state)
